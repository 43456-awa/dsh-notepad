...
function apply(ctx) {
  ...
  /** 工具侧作用域解析：scope=session 但缺 sessionId 时给出可用会话页提示。 */
  function resolveToolScope(args) {
    const key = scopeKeyOf(args.scope, args.sessionId);
    if (args.scope === "session" && key === SCOPE_GLOBAL) {
      const pages = listSessionScopes();
      const hint = pages.length === 0
        ? "（目前没有任何会话隔离页）"
        : `现有会话页：${pages.map((p) => `"${p}"`).join(", ")}`;
      throw new Error(`会话作用域需要 sessionId 参数（在对应会话里打开面板写入后即自动创建）。${hint}`);
    }
    return key;
  }
  ...
  tools.register(defineTool({
    name: "notepad_read",
    description: "读取钉在 dsh Web GUI 右侧的常驻记事本内容（用户便签，纯文本，存于 ~/.dsh/notepad/）。scope=session 时读取指定会话的隔离记事本页。",
    parameters: {
      scope: { type: "string", description: "作用域：global（默认，全部会话共享）或 session（会话隔离页）" },
      sessionId: { type: "string", description: "scope=session 时必填：目标会话 id（可用 notepad_read scope=session 查看现有会话页列表）" }
    },
    output: { schema: { type: "string" }, render: (_args, value) => [{ type: "text", text: String(value) }] },
    async execute(args) {
      const key = resolveToolScope(args ?? {});
      const text = readNotes(key);
  ...
  tools.register(defineTool({
    name: "notepad_write",
    description: "写入/追加常驻记事本（钉在 dsh Web GUI 右侧的便签，存于 ~/.dsh/notepad/）。mode=replace 覆盖全文；mode=append 在末尾追加且按行去重。timestamp=true 时给每行加 [MM-DD HH:mm] 前缀。scope=session 时写入指定会话的隔离记事本页。",
    parameters: {
      ...
      scope: { type: "string", description: "作用域：global（默认，全部会话共享）或 session（会话隔离页）" },
      sessionId: { type: "string", description: "scope=session 时必填：目标会话 id" }
    },
    output: { schema: { type: "string" }, render: (_args, value) => [{ type: "text", text: String(value) }] },
    async execute(args) {
      const key = resolveToolScope(args);
      ...
      if (args.mode === "append") {
        ...
        return `已追加 ${fresh.length} 行，跳过 ${lines.length - fresh.length} 行重复。`;
      }
  ...
  tools.register(defineTool({
    name: "notepad_search",
    description: "在记事本中按关键词搜索（不区分大小写子串匹配）。默认搜索全局页 + 所有会话隔离页，可限定作用域。返回匹配行、所在页与行号。",
    parameters: {
      query: { type: "string", required: true, description: "搜索关键词" },
      scope: { type: "string", description: "限定作用域：global 或 session（省略则搜全部）" },
      sessionId: { type: "string", description: "scope=session 时必填：目标会话 id" },
    ...
    async execute(args) {
      ...
      if (args.scope === "session" || args.scope === "global") {
        const key = scopeKeyOf(args.scope, args.sessionId);
        if (args.scope === "session" && key === SCOPE_GLOBAL) resolveToolScope(args); // 缺 sessionId 时给出提示
        targets.push(key);
      } else {
